...
<div class="bgm-crm-dark" style="margin: -16px -15px -40px -15px;padding: 30px 46px 70px">
    <div class="row">
        <div class="col-lg-3 col-xs-12">
            <div class="c-white text-nowrap m-b-5">
                <i class="zmdi zmdi-phone p-r-5 p-t-3 pull-left"></i>
                <a class="c-white" href="tel:{{ $client->phone or '' }}">{{ $client->phone }}</a>
            </div>
            <div class="c-white text-nowrap m-b-5">
                <i class="zmdi zmdi-email p-r-5 p-t-4 pull-left"></i>
                <a class="c-white" href="mailto:{{ $client->email or '' }}">{{ $client->email }}</a>
            </div>
            @if(config('crm.mode') === 'mechanic')
                <div class="c-white text-nowrap m-b-5">
                    <i class="zmdi zmdi-assignment p-r-5 p-t-3 pull-left"></i>
                    Α.Φ.Μ.: {{ $client->vat }}
                </div>
            @endif
            <div class="c-white text-nowrap">
                <i class="zmdi zmdi-comment-text p-r-5 p-t-3 pull-left"></i>
                {!! $client->notes ? nl2br($client->notes) : 'Χωρίς σημειώσεις' !!}
            </div>
        </div>
        @if(config('crm.mode') === 'mechanic')
            <div class="col-lg-3 col-xs-12">
                <hr class="darkbg hidden-lg" />
                <div class="c-white f-700 m-b-5">
                    {{ $client->vehicle->brand }}{!! $client->vehicle->model ? ' – ' . $client->vehicle->model : '' !!}{!! $client->vehicle->model ? ' του ' . $client->vehicle->production_year : '' !!}
                </div>
                <div class="c-white m-b-5">
                    {!! $client->vehicle->engine_displacement_cc ? 'Κυβικά: ' . $client->vehicle->engine_displacement_cc . ' εκ.' : '' !!}
                </div>
                <div class="c-white m-b-5">
                    {!! $client->vehicle->vin ? 'Αρ. πλαισίου: ' . $client->vehicle->vin : '' !!}
                </div>
                <div class="c-white m-b-5">
                    {!! $client->vehicle->license_plate ? 'Αρ. Κυκλοφορίας: ' . $client->vehicle->license_plate : '' !!}
                </div>
                <div class="c-white m-b-5">
                    {!! $client->vehicle->engine_code ? 'Κινητήρας: ' . $client->vehicle->engine_code : '' !!}
                </div>
            </div>
        @endif
    </div>
                            ...
                            <tr>
                                <td>
                                    @if(Auth::user()->role === 'admin')
                                        <a href="{{ route('client.labor.destroy', ['id' => $client->id, 'lid' => $lab->id]) }}" class="c-orange p-r-15 dialog-confirm-delete">
                                            <i class="zmdi zmdi-delete f-s-18"></i>
                                        </a>
                                    @endif
                                    <a href="{{ route('client.labor.edit', ['id' => $client->id, 'lid' => $lab->id]) }}" class="c-black">
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on('click', '.dialog-confirm-delete', function (e) {
                e.preventDefault();

                if (confirm('Να γίνει διαγραφή;')) {
